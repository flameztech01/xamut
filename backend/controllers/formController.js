...
const sendParticipantInviteEmail = async ({ form, participant, password }) => {
  ...
  const html = `
    <div style="font-family:Raleway,Arial,sans-serif;max-width:520px;margin:0 auto;padding:24px;color:#1c1917;">
      ...
      <div style="background:#fafaf9;border:1px solid #e7e5e4;border-radius:10px;padding:14px 16px;margin:0 0 20px;">
...
const sendCollaboratorInviteEmail = async ({
  ...
  const roleBlurb =
    role === "editor"
      ? "As an editor you can update the form, manage settings, and see all responses."
      : "As a viewer you can look at the form and its responses, but you can't edit it.";
  ...
  const subject = `You've been invited to ${
  ...
  const text = [
    `Hi ${inviteName},`,
    "",
    `${inviterName} invited you as ${roleLabel} on "${form.title}".`,
    "",
    roleBlurb,
    "",
  ...
  const html = `
    <div style="font-family:Raleway,Arial,sans-serif;max-width:520px;margin:0 auto;padding:24px;color:#1c1917;">
      <h2 style="margin:0 0 12px;font-size:20px;">You've been invited to a form</h2>
      <p style="margin:0 0 16px;color:#44403c;">
        Hi ${inviteName}, ${inviterName} invited you as
...
export const addCollaborator = asyncHandler(async (req, res) => {
  ...
  if (existingPending) {
    ...
    if (name) existingPending.name = String(name).slice(0, 120);
    existingPending.lastInvitedAt = new Date();
...
export const listCollaborators = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({
    ...
    pendingCollaborators: (form.pendingCollaborators || []).map((p) => ({
      email: p.email,
      name: p.name || "",
      role: p.role,
      invitedAt: p.invitedAt,
